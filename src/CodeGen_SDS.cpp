...
namespace Halide {
    namespace Internal {
        ...
        CodeGen_SDS::CodeGen_SDS(ostream &s, OutputKind output_kind, const std::string &guard) : IRPrinter(s),
                                                                                                 id("$$ BAD ID $$"),
                                                                                                 output_kind(
                                                                                                         output_kind),
                                                                                                 extern_c_open(false) {
        ...
        namespace {
            class ExternCallPrototypes : public IRGraphVisitor {
                ...
                void
                emit_namespace_or_call(ostream &stream, const NamespaceOrCall &ns_or_call, const std::string &name) {
        ...
        void CodeGen_SDS::compile(const LoweredFunc &f) {
                ...
                if (args[i].type.handle_type != NULL) {
                    if (!args[i].type.handle_type->namespaces.empty()) {
                        if (args[i].type.handle_type->inner_name.cpp_type_type != halide_cplusplus_type_name::Simple) {
                            ...
                            if (args[i].type.handle_type->inner_name.cpp_type_type !=
            ...
            if (is_header()) {
                ...
                stream << "const struct halide_filter_metadata_t *" << simple_name
                       << "_metadata() HALIDE_FUNCTION_ATTRS;\n";
            }
        ...
        void CodeGen_SDS::compile(const Buffer<> &buffer) {
            ...
            user_assert(!b.dev_dirty) << "Can't embed image: " << buffer.name()
...
// (dsharlet).
        ...
        void CodeGen_SDS::visit(const Call *op) {
            ...
            if (op->is_intrinsic(Call::sds_tmp_access)) {
                ...
                id = "0";
                return;
            } else if (op->is_intrinsic(Call::sds_linebuffer_access) ||
                       op->is_intrinsic(Call::sds_windowbuffer_access)) {
        ...
        void CodeGen_SDS::visit(const Allocate *op) {
            ...
            if (starts_with(op->name, "dup$$")) {

                Allocation alloc;
                alloc.type = op->type;
                allocations.push(op->name, alloc);

                int buffer_size = op->constant_allocation_size();
                internal_assert(buffer_size > 0);
                stream << "#ifdef __SDSCC__\n";
                do_indent();
                ...
                if (op->new_expr.defined()) {
                    ...
                    if (constant_size > 0) {
                        ...
                        for (size_t i = 1; i < op->extents.size(); i++) {
                            ...
                            if (i > 1) {
                                new_size_id_rhs =
                                        "(" + size_id + " > ((int64_t(1) << 31) - 1)) ? " + size_id + " : (" + size_id +
                                        " * " + next_extent + ")";
                            } else {
                    ...
                    if (on_stack) {
                        ...
                        if (is_hardware()) {
                            do_indent();
                            stream << "#pragma HLS array_partition variable=" << print_name(op->name)
                                   << " complete dim=0\n";
                        }
                    } else {
                        //if (is_hardware()) {
